...
class TravelService extends cds_1.ApplicationService {
    init() {
        ...
        this.before('CREATE', TravelService_1.Travel, async (req) => {
            // FIXME: TS v
            const { maxID } = await SELECT.one `max(TravelID) as maxID`.from(TravelService_1.Travel);
        ...
        this.before('NEW', TravelService_1.Booking.drafts, async (req) => {
            ...
            const { status } = await SELECT.one.from(TravelService_1.Travel.drafts, to_Travel_TravelUUID, t => t.TravelStatus_code.as('status'));
            if (status === travel_1.TravelStatusCode.Canceled)
                throw req.reject(400, 'Cannot add new bookings to rejected travels.');
            // FIXME: TS v
            const { maxID } = await SELECT.one `max(BookingID) as maxID`.from(TravelService_1.Booking.drafts).where({ to_Travel_TravelUUID });
        ...
        this.before('NEW', TravelService_1.BookingSupplement.drafts, async (req) => {
            ...
            // FIXME: TS v
            const { maxID } = await SELECT.one `max(BookingSupplementID) as maxID`.from(TravelService_1.BookingSupplement.drafts).where({ to_Booking_BookingUUID });
        ...
        this.before('UPDATE', TravelService_1.Travel.drafts, async (req) => {
            if ('BookingFee' in req.data) {
                const { TravelStatus_code: status } = await SELECT.one.from(req.subject);
                if (status === travel_1.TravelStatusCode.Accepted)
                    req.reject(400, 'Booking fee can not be updated for accepted travels.', 'BookingFee');
            }
        });
        /**
         ...
         */
        this.after('UPDATE', TravelService_1.BookingSupplement.drafts, async (_, req) => {
            if ('Price' in req.data) {
                const { BookSupplUUID } = req.data;
                // We need to fetch the Travel's UUID for the given Supplement target
                const BookingUUID = SELECT.one(TravelService_1.BookingSupplement.drafts, b => b.to_Booking_BookingUUID).where({ BookSupplUUID });
                const { to_Travel_TravelUUID: travel } = await SELECT.one(TravelService_1.Booking.drafts, b => b.to_Travel_TravelUUID).where({ BookingUUID });
                return this._update_totals4(travel);
            }
        });
        /**
         ...
         */
        this.on('CANCEL', TravelService_1.BookingSupplement.drafts, async (req, next) => {
            // Find out which travel is affected before the delete
            const { BookSupplUUID } = req.data;
            const { to_Travel_TravelUUID } = await SELECT.one
                .from(TravelService_1.BookingSupplement.drafts, b => b.to_Travel_TravelUUID)
                .where({ BookSupplUUID });
        ...
        this.before('SAVE', TravelService_1.Travel, req => {
            ...
            if (TravelStatus_code !== travel_1.TravelStatusCode.Canceled) {
                ...
                if (!BeginDate)
                    req.error(400, "Enter a begin date", "in/BeginDate");
                if (!EndDate)
                    req.error(400, "Enter an end date", "in/EndDate");
                ...
                for (const booking of to_Booking) {
                    ...
                    if (!ConnectionID)
                        req.error(400, "Enter a flight", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/ConnectionID`);
                    if (!FlightDate)
                        req.error(400, "Enter a flight date", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/FlightDate`);
                    if (!FlightPrice)
                        req.error(400, "Enter a flight price", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/FlightPrice`);
                    if (!BookingStatus_code)
                        req.error(400, "Enter a booking status", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/BookingStatus_code`);
                    if (!to_Carrier_AirlineID)
                        req.error(400, "Enter an airline", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/to_Carrier_AirlineID`);
                    if (!to_Customer_CustomerID)
                        req.error(400, "Enter a customer", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/to_Customer_CustomerID`);
                    for (const suppl of booking.to_BookSupplement) {
                        const { BookSupplUUID, Price, to_Supplement_SupplementID } = suppl;
                        if (!Price)
                            req.error(400, "Enter a price", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/to_BookSupplement(BookSupplUUID='${BookSupplUUID}',IsActiveEntity=false)/Price`);
                        if (!to_Supplement_SupplementID)
                            req.error(400, "Enter a supplement", `in/to_Booking(BookingUUID='${BookingUUID}',IsActiveEntity=false)/to_BookSupplement(BookSupplUUID='${BookSupplUUID}',IsActiveEntity=false)/to_Supplement_SupplementID`);
                    }
                }
            }
            if (BeginDate < today)
                req.error(400, `Begin Date ${BeginDate} must not be before today ${today}.`, 'in/BeginDate');
            if (BeginDate > EndDate)
                req.error(400, `Begin Date ${BeginDate} must be before End Date ${EndDate}.`, 'in/BeginDate');
        });
        //
        // Action Implementations...
        //
        const { acceptTravel, rejectTravel, deductDiscount } = TravelService_1.Travel.actions;
        this.on(acceptTravel, req => UPDATE(req.subject).with({ TravelStatus_code: travel_1.TravelStatusCode.Accepted }));
        this.on(rejectTravel, req => UPDATE(req.subject).with({ TravelStatus_code: travel_1.TravelStatusCode.Canceled }));
